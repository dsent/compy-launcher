...
- **Startup storage**: Launcher waits up to 30 seconds for Android to mount the SD card before checking it and recovering interrupted restores. If the mounted card cannot be read or written yet, Launcher checks it again every 3 seconds for up to 30 seconds. A card that remains unavailable shows the storage warning; Maintenance remains accessible.
